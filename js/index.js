window.onload = function() {
	...
	function nss() {
		...
		let flag = true;
		$(".aside-left li").click(function() {
			flag = false;
			let index = $(".aside-left li").index(this)
			let Top = $(".louceng").eq(index).offset().top;
			$("html").animate({
				scrollTop: Top
			}, 500, function() {
				flag = true;
		...
		$(window).scroll(function() {
			...
			if (flag) {
				$(".louceng").each(function(index, val) {
					let bol = (index === 0 || $(".louceng").eq(index - 1).offset().top < scrollTop)
					if ($(".louceng").eq(index).offset().top >= scrollTop && bol) {
						$(".aside-left li").eq(index).addClass("active").siblings().removeClass("active");
					} //从4的距离到5的距离有缺陷(如果5的离底部距离过大时)
				})
				if (scrollTop > $(".louceng").eq(0).offset().top - window.innerHeight) {
					barx.css("display", "block");
				} else {
					barx.css("display", "none");
				}
